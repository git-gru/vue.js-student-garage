...
let systemFirms = [
  { name: "Founders Fund", emailSuffix: "@foundersfund.com" },
  { name: "A16Z", emailSuffix: "@a16z.com" },
  { name: "Greylock", emailSuffix: "@greylock.com" },
  { name: "Lightspeed", emailSuffix: "@lsvp.com" }
];
export default {
  ...
  methods: {
    ...
    invalidPasswordTyping() {
      if (
        ...
        (this.userPassword.length < 8 || this.userPassword.trim().length == 0)
      )
        return true;
      return false;
    },
    ...
    emailSuffixIssue() {
      ...
      let userSuffix = this.userEmail.substring(
        this.userEmail.length - correctSuffix.length,
